...
public class Board {
    ...
    public Board(int width, int height) {
        ...
        this.width = width;
        this.height = height;
        // if (entityGrid != null) this.entityGrid = new Entity[height][width];
    }
    ...
    public Position generateRandomPosition() {
        // Entity.random is accessible as Entity is in the same package or imported
        int x = Entity.random.nextInt(width);
        int y = Entity.random.nextInt(height);
        return new Position(x, y);
    }

    // Grid-related methods (if implemented):
